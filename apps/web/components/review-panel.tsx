...
export function ReviewPanel() {
  const [repositoryId, setRepositoryId] = useState(""); const [pr, setPr] = useState(""); const [status, setStatus] = useState("Local review queue only; GitHub is disconnected.");
  async function submit(event: FormEvent<HTMLFormElement>) { event.preventDefault(); try { const response = await fetch(`${apiBase}/api/reviews`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ repository_id: repositoryId, pull_request_number: Number(pr) }) }); const payload = await response.json() as { message?: string; detail?: string }; setStatus(response.ok ? payload.message ?? "Queued" : payload.detail ?? "Unable to queue review"); } catch { setStatus("Local API is unavailable"); } }
